...
ser_cred - Tool for low-level credential management.
...
Command 'B<ser_cred add>' add new credentials into ser database. (Appropriate
hashes are generated automaticaly.)

Command 'B<ser_domain change>' change password or flags for I<auth_username>
...
regenerated automaticaly.)

Command 'B<ser_cred rm>' mark credentials  as deleted
(set DELETED flag).

Command 'B<ser_cred disable>' set DISABLED flag for specified credentials.

Command 'B<ser_cred enable>' unset DISABLED flag for specified credentials.

Command 'B<ser_cred purge>' remove all credentials marked as deleted from
database.

Command 'B<ser_cred show>' print credentials.
