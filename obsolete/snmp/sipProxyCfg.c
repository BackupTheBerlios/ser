/*
 * $Id: sipProxyCfg.c,v 1.2 2002/09/19 12:23:55 jku Rel $
 *
 * SNMP Module
 *
...
/* Table sipProxyCfgTable */
static netsnmp_table_data_set* sipProxyCfgTable;

static int initialize_table_sipProxyCfgTable();
static Netsnmp_Node_Handler sipProxyCfgTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipProxyCfgTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipProxyCfgTable_gh;
/* The registration function and friends */
static int sipProxyCfgTable_reg(struct sip_snmp_handler *h, int op);
static int sipProxyCfgTable_addObj(struct sip_snmp_handler *h, int col);
static int sipProxyCfgTable_newRow(struct sip_snmp_handler *h);

/** Initializes the sipProxyCfg module */
int init_sipProxyCfg()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipProxyCfgTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipProxyCfgTable\n", func);
		return -1;
	}
	return 0;
}
...
{
	const char *func = "snmp_mod";
	sipProxyCfgTable_h = calloc(SIPPROXYCFGTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipProxyCfgTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipProxyCfgTable_reg;
}
...
static int sipProxyCfgTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPPROXYCFGTABLE_COLUMNS) {
		...
		if(!(sipProxyCfgTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipProxyCfgTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipProxyCfgTable_h[col]);
			sipProxyCfgTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipProxyCfgTable_gh && first) {
			...
			c = h;
			if(SIPPROXYCFGTABLE_INDEXES > 1) {
				for(i=1; i<SIPPROXYCFGTABLE_INDEXES; i++)
					c = c->next;
			...
			while(c) {
				...
				if(col > SIPPROXYCFGTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipProxyCfgTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipProxyCfgTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipProxyCfgTable_gh)
			sipProxyCfgTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipProxyCfgTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipProxyCfgTable_gh) {
		...
		if(!sipProxyCfgTable_gh->sip_obj) {
			sipProxyCfgTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipProxyCfgTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipProxyCfgTable_gh);
				sipProxyCfgTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipProxyCfgTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipProxyCfgTable->table, row) != 
				SNMPERR_SUCCESS) {
...
static int sipProxyCfgTable_newRow(struct sip_snmp_handler *h)
{
	...
	c = h;
	for(i=2; i<SIPPROXYCFGTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPPROXYCFGTABLE_COLUMNS) {
...
int sipProxyCfgTable_replaceRow(
		...
		struct sip_snmp_obj *data)
{
	...
	/* find row */
	row = netsnmp_table_data_get(sipProxyCfgTable->table, idxs);
	if(!row) {
