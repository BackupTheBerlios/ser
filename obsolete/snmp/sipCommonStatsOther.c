/*
 * $Id: sipCommonStatsOther.c,v 1.2 2002/09/19 12:23:54 jku Rel $
 *
 * SNMP Module
 *
...
/* Table sipOtherStatsTable */
static netsnmp_table_data_set* sipOtherStatsTable;

static int initialize_table_sipOtherStatsTable();
static Netsnmp_Node_Handler sipOtherStatsTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipOtherStatsTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipOtherStatsTable_gh;
/* The registration function and friends */
static int sipOtherStatsTable_reg(struct sip_snmp_handler *h, int op);
...
/** Initializes the sipCommonStatsOther module */
int init_sipCommonStatsOther()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipOtherStatsTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipOtherStatsTable\n", func);
		return -1;
	}
	return 0;
}
...
{
	const char *func = "snmp_mod";
	sipOtherStatsTable_h = calloc(SIPOTHERSTATSTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipOtherStatsTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipOtherStatsTable_reg;
}
...
static int sipOtherStatsTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		if(!(sipOtherStatsTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipOtherStatsTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipOtherStatsTable_h[col]);
			sipOtherStatsTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipOtherStatsTable_gh && first) {
			...
			c = h;
			if(SIPOTHERSTATSTABLE_INDEXES > 1) {
			...
			while(c) {
				...
				c->sip_obj->col = col;
				if(!(sipOtherStatsTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipOtherStatsTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipOtherStatsTable_gh)
			sipOtherStatsTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipOtherStatsTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipOtherStatsTable_gh) {
		...
		if(!sipOtherStatsTable_gh->sip_obj) {
			sipOtherStatsTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipOtherStatsTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipOtherStatsTable_gh);
				sipOtherStatsTable_gh = NULL;
				return -1;
			}
		}
	} else {
