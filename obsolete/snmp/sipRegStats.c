/*
 * $Id: sipRegStats.c,v 1.2 2002/09/19 12:23:55 jku Rel $
 *
 * SNMP Module
 *
...
/* Table sipRegStatsTable */
static netsnmp_table_data_set* sipRegStatsTable;

static int initialize_table_sipRegStatsTable();
static Netsnmp_Node_Handler sipRegStatsTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipRegStatsTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipRegStatsTable_gh;
/* The registration function and friends */
static int sipRegStatsTable_reg(struct sip_snmp_handler *h, int op);
static int sipRegStatsTable_addObj(struct sip_snmp_handler *h, int col);
static int sipRegStatsTable_newRow(struct sip_snmp_handler *h);

/** Initializes the sipRegStats module */
int init_sipRegStats()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipRegStatsTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipRegStatsTable\n", func);
		return -1;
	}
	return 0;
}

/** Initialize the sipRegStatsTable table by defining it's contents and 
 * how it's structured */
static int initialize_table_sipRegStatsTable()
{
	static oid sipRegStatsTable_oid[] = {1,3,6,1,2,1,9991,1,8,1};
	size_t sipRegStatsTable_oid_len = OID_LENGTH(sipRegStatsTable_oid);
	const char *func = "snmp_mod";

	/* create the table structure itself */
	sipRegStatsTable = netsnmp_create_table_data_set("sipRegStatsTable");
	if(!sipRegStatsTable) {
	 ...
	 * Adding indexes
	 */
	netsnmp_table_dataset_add_index(sipRegStatsTable, ASN_INTEGER);
	
	netsnmp_table_set_multi_add_default_row(
		sipRegStatsTable,
		COLUMN_SIPREGACCEPTEDREGISTRATIONS, ASN_COUNTER, 0, NULL, 0,
		COLUMN_SIPREGREJECTEDREGISTRATIONS, ASN_COUNTER, 0, NULL, 0,
		0);
	
	/* registering the table with the master agent */
	/* note: if you don't need a subhandler to deal with any aspects
	 * of the request, change sipRegStatsTable_handler to "NULL" */
	netsnmp_register_table_data_set(
			netsnmp_create_handler_registration(
				"sipRegStatsTable", 
				sipRegStatsTable_handler,
				sipRegStatsTable_oid,
				sipRegStatsTable_oid_len,
				HANDLER_CAN_RWRITE),
			sipRegStatsTable, NULL);
...
{
	const char *func = "snmp_mod";
	sipRegStatsTable_h = calloc(SIPREGSTATSTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipRegStatsTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipRegStatsTable_reg;
}
...
static int sipRegStatsTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPREGSTATSTABLE_COLUMNS) {
		...
		if(!(sipRegStatsTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipRegStatsTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipRegStatsTable_h[col]);
			sipRegStatsTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipRegStatsTable_gh && first) {
			...
			c = h;
			if(SIPREGSTATSTABLE_INDEXES > 1) {
				for(i=1; i<SIPREGSTATSTABLE_INDEXES; i++)
					c = c->next;
			...
			while(c) {
				...
				if(col > SIPREGSTATSTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipRegStatsTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipRegStatsTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipRegStatsTable_gh)
			sipRegStatsTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipRegStatsTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipRegStatsTable_gh) {
		...
		if(!sipRegStatsTable_gh->sip_obj) {
			sipRegStatsTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipRegStatsTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipRegStatsTable_gh);
				sipRegStatsTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipRegStatsTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipRegStatsTable->table, row) != 
				SNMPERR_SUCCESS) {
...
static int sipRegStatsTable_newRow(struct sip_snmp_handler *h)
{
	...
	c = h;
	for(i=2; i<SIPREGSTATSTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPREGSTATSTABLE_COLUMNS) {
...
int sipRegStatsTable_replaceRow(
		...
		struct sip_snmp_obj *data)
{
	...
	/* find row */
	row = netsnmp_table_data_get(sipRegStatsTable->table, idxs);
	if(!row) {
		...
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		if(table_info->colnum > SIPREGSTATSTABLE_COLUMNS)
			goto next;

		/* Get the handler and its object */
		if(sipRegStatsTable_gh) {
			h = sipRegStatsTable_gh;
			/* sip_obj is valid since we create upon registration */
			h->sip_obj->opaque = (void*)sipRegStatsTable_replaceRow;
		} else {
			h = sipRegStatsTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
