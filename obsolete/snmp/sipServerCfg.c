/*
 * $Id: sipServerCfg.c,v 1.2 2002/09/19 12:23:55 jku Rel $
 *
 * SNMP Module
 *
...
/* Table sipServerCfgTable */
static netsnmp_table_data_set* sipServerCfgTable;

static int initialize_table_sipServerCfgTable();
static Netsnmp_Node_Handler sipServerCfgTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipServerCfgTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipServerCfgTable_gh;
/* The registration function and friends */
static int sipServerCfgTable_reg(struct sip_snmp_handler *h, int op);
static int sipServerCfgTable_addObj(struct sip_snmp_handler *h, int col);
static int sipServerCfgTable_newRow(struct sip_snmp_handler *h);

/** Initializes the sipServerCfg module */
int init_sipServerCfg()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipServerCfgTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipServerCfgTable\n", func);
		return -1;
	}
	return 0;
}

/** Initialize the sipServerCfgTable table by defining it's contents and 
 * how it's structured */
static int initialize_table_sipServerCfgTable()
{
	static oid sipServerCfgTable_oid[] = {1,3,6,1,2,1,9991,1,1,1};
	size_t sipServerCfgTable_oid_len = OID_LENGTH(sipServerCfgTable_oid);
	const char *func = "snmp_mod";

	/* create the table structure itself */
	sipServerCfgTable = netsnmp_create_table_data_set("sipServerCfgTable");
	if(!sipServerCfgTable) {
	 ...
	 * Adding indexes
	 */
	netsnmp_table_dataset_add_index(sipServerCfgTable, ASN_INTEGER);
	
	netsnmp_table_set_multi_add_default_row(
		sipServerCfgTable,
		COLUMN_SIPSERVERHOSTADDRTYPE, ASN_INTEGER, 1, NULL, 0,
		COLUMN_SIPSERVERHOSTADDR, ASN_OCTET_STR, 1, NULL, 0,
		COLUMN_SIPPGPVERSION, ASN_OCTET_STR, 0, NULL, 0,
		COLUMN_SIPSERVERCONTACTDFLTACTION, ASN_INTEGER, 1, NULL, 0,
		COLUMN_SIPSERVERRESPECTUAACTION, ASN_INTEGER, 1, NULL, 0,
		0);
	
	/* registering the table with the master agent */
	/* note: if you don't need a subhandler to deal with any aspects
	 * of the request, change sipServerCfgTable_handler to "NULL" */
	netsnmp_register_table_data_set(
			netsnmp_create_handler_registration(
				"sipServerCfgTable", 
				sipServerCfgTable_handler,
				sipServerCfgTable_oid,
				sipServerCfgTable_oid_len,
				HANDLER_CAN_RWRITE),
			sipServerCfgTable, NULL);
...
{
	const char *func = "snmp_mod";
	sipServerCfgTable_h = calloc(SIPSERVERCFGTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipServerCfgTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipServerCfgTable_reg;
}
...
static int sipServerCfgTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPSERVERCFGTABLE_COLUMNS) {
		...
		if(!(sipServerCfgTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipServerCfgTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipServerCfgTable_h[col]);
			sipServerCfgTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipServerCfgTable_gh && first) {
			...
			c = h;
			if(SIPSERVERCFGTABLE_INDEXES > 1) {
				for(i=1; i<SIPSERVERCFGTABLE_INDEXES; i++)
					c = c->next;
			...
			while(c) {
				...
				if(col > SIPSERVERCFGTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipServerCfgTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipServerCfgTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipServerCfgTable_gh)
			sipServerCfgTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipServerCfgTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipServerCfgTable_gh) {
		...
		if(!sipServerCfgTable_gh->sip_obj) {
			sipServerCfgTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipServerCfgTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipServerCfgTable_gh);
				sipServerCfgTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipServerCfgTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipServerCfgTable->table, row) != 
				SNMPERR_SUCCESS) {
...
static int sipServerCfgTable_newRow(struct sip_snmp_handler *h)
{
	...
	c = h;
	for(i=2; i<SIPSERVERCFGTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPSERVERCFGTABLE_COLUMNS) {
...
int sipServerCfgTable_replaceRow(
		...
		struct sip_snmp_obj *data)
{
	...
	/* find row */
	row = netsnmp_table_data_get(sipServerCfgTable->table, idxs);
	if(!row) {
		...
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		if(table_info->colnum > SIPSERVERCFGTABLE_COLUMNS)
			goto next;

		/* Get the handler and its object */
		if(sipServerCfgTable_gh) {
			h = sipServerCfgTable_gh;
			/* sip_obj is valid since we create upon registration */
			h->sip_obj->opaque = (void*)sipServerCfgTable_replaceRow;
		} else {
			h = sipServerCfgTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
