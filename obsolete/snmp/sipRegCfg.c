...
/* Table sipRegCfgTable */
static netsnmp_table_data_set* sipRegCfgTable;

static int initialize_table_sipRegCfgTable();
static Netsnmp_Node_Handler sipRegCfgTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipRegCfgTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipRegCfgTable_gh;
/* The registration function and friends */
static int sipRegCfgTable_reg(struct sip_snmp_handler *h, int op);
static int sipRegCfgTable_addObj(struct sip_snmp_handler *h, int col);
static int sipRegCfgTable_newRow(struct sip_snmp_handler *h);

/* Table sipContactTable */
static netsnmp_table_data_set* sipContactTable;

static int initialize_table_sipContactTable();
static Netsnmp_Node_Handler sipContactTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipContactTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipContactTable_gh;
/* The registration function and friends */
static int sipContactTable_reg(struct sip_snmp_handler *h, int op);
static int sipContactTable_addObj(struct sip_snmp_handler *h, int col);
static int sipContactTable_newRow(struct sip_snmp_handler *h);

/* Table sipRegUserTable */
static netsnmp_table_data_set* sipRegUserTable;

static int initialize_table_sipRegUserTable();
static Netsnmp_Node_Handler sipRegUserTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipRegUserTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipRegUserTable_gh;
/* The registration function and friends */
static int sipRegUserTable_reg(struct sip_snmp_handler *h, int op);
static int sipRegUserTable_addObj(struct sip_snmp_handler *h, int col);
static int sipRegUserTable_newRow(struct sip_snmp_handler *h);

/** Initializes the sipRegCfg module */
int init_sipRegCfg()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipRegCfgTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipRegCfgTable\n", func);
		return -1;
	}
	if(initialize_table_sipContactTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipContactTable\n", func);
		return -1;
	}
	if(initialize_table_sipRegUserTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipRegUserTable\n", func);
		return -1;
	}
	return 0;
}
...
{
	const char *func = "snmp_mod";
	sipRegCfgTable_h = calloc(SIPREGCFGTABLE_COLUMNS+1,
...
static int sipRegCfgTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPREGCFGTABLE_COLUMNS) {
		...
		if(!(sipRegCfgTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipRegCfgTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipRegCfgTable_h[col]);
			sipRegCfgTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipRegCfgTable_gh && first) {
			...
			c = h;
			if(SIPREGCFGTABLE_INDEXES > 1) {
				for(i=1; i<SIPREGCFGTABLE_INDEXES; i++)
					c = c->next;
			...
			while(c) {
				...
				if(col > SIPREGCFGTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipRegCfgTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipRegCfgTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipRegCfgTable_gh)
			sipRegCfgTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipRegCfgTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipRegCfgTable_gh) {
		...
		if(!sipRegCfgTable_gh->sip_obj) {
			sipRegCfgTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipRegCfgTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipRegCfgTable_gh);
				sipRegCfgTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipRegCfgTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipRegCfgTable->table, row) != 
				SNMPERR_SUCCESS) {
...
static int sipRegCfgTable_newRow(struct sip_snmp_handler *h)
{
	...
	c = h;
	for(i=2; i<SIPREGCFGTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPREGCFGTABLE_COLUMNS) {
		...
		struct sip_snmp_obj *data)
{
	...
	/* find row */
	row = netsnmp_table_data_get(sipRegCfgTable->table, idxs);
	if(!row) {
		...
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		if(table_info->colnum > SIPREGCFGTABLE_COLUMNS)
			goto next;
		...
		if(sipRegCfgTable_gh) {
			...
			h->sip_obj->opaque = (void*)sipRegCfgTable_replaceRow;
		} else {
			h = sipRegCfgTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
...
{
	const char *func = "snmp_mod";
	sipContactTable_h = calloc(SIPCONTACTTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipContactTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipContactTable_reg;
}
...
static int sipContactTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPCONTACTTABLE_COLUMNS) {
			LOG(L_ERR, "%s: attempt to register invalid column %d\n", 
				func, col);
			return -1;
		}
		...
		/* add handler to table. We make copy to make everybody's life easier 
		 * (note that value is not copied) */
		if(!(sipContactTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipContactTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipContactTable_h[col]);
			sipContactTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		/* if there's global handler, no need to add the handlers */
		if(!sipContactTable_gh && first) {
			/* first time around, jump over indexes, and copy the handlers.
			 * (Indexes don't have handlers associated with them). The default
			 * index (e.g. applIndex) shouldn't be passed in, that's why
			 * we jump over one less index. */
			c = h;
			if(SIPCONTACTTABLE_INDEXES > 1) {
				for(i=1; i<SIPCONTACTTABLE_INDEXES; i++)
					c = c->next;
				col = 2;	/* 2nd or 3rd index will be at column 1 */
			} else
				col = 1;
			while(c) {
				...
				c->sip_obj->col = col;
				if(!(sipContactTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipContactTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipContactTable_gh)
			sipContactTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipContactTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipContactTable_gh) {
			LOG(L_ERR, "%s: Error registering table handler\n", func);
			return -1;
		}
		/* it's not necessary for the caller to create this, but we need
		 * it to pass info down to the handling function */
		if(!sipContactTable_gh->sip_obj) {
			sipContactTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipContactTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipContactTable_gh);
				sipContactTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipContactTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipContactTable->table, row) != 
				SNMPERR_SUCCESS) {
			LOG(L_ERR, "%s: Error adding new row to table\n",func); 
			return -1;
		}
...
static int sipContactTable_newRow(struct sip_snmp_handler *h)
{
	 ...
	 * end of the linked list */
	c = h;
	for(i=2; i<SIPCONTACTTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPCONTACTTABLE_COLUMNS) {
...
/** handles requests for the sipContactTable table.
 * For every request it checks the specified object to see if it has a
 * handler, and calls it */
static int sipContactTable_handler(
		netsnmp_mib_handler               *handler,
		netsnmp_handler_registration      *reginfo,
		netsnmp_agent_request_info        *reqinfo,
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		/* this is not an error, since table-walks work by trying to get
		 * things until we run off of it */
		if(table_info->colnum > SIPCONTACTTABLE_COLUMNS)
			goto next;

		/* Get the handler and its object */
		if(sipContactTable_gh) {
			h = sipContactTable_gh;
			/* sip_obj is valid since we create upon registration */
			h->sip_obj->opaque = (void*)sipContactTable_replaceRow;
		} else {
			h = sipContactTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
...
{
	const char *func = "snmp_mod";
	sipRegUserTable_h = calloc(SIPREGUSERTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipRegUserTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipRegUserTable_reg;
}
...
static int sipRegUserTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPREGUSERTABLE_COLUMNS) {
			LOG(L_ERR, "%s: attempt to register invalid column %d\n", 
				func, col);
			return -1;
		}
		...
		/* add handler to table. We make copy to make everybody's life easier 
		 * (note that value is not copied) */
		if(!(sipRegUserTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipRegUserTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipRegUserTable_h[col]);
			sipRegUserTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		/* if there's global handler, no need to add the handlers */
		if(!sipRegUserTable_gh && first) {
			/* first time around, jump over indexes, and copy the handlers.
			 * (Indexes don't have handlers associated with them). The default
			 * index (e.g. applIndex) shouldn't be passed in, that's why
			 * we jump over one less index. */
			c = h;
			if(SIPREGUSERTABLE_INDEXES > 1) {
				for(i=1; i<SIPREGUSERTABLE_INDEXES; i++)
					c = c->next;
				col = 2;	/* 2nd or 3rd index will be at column 1 */
			} else
				col = 1;
			while(c) {
				/* if too many we ignore the rest */
				if(col > SIPREGUSERTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipRegUserTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipRegUserTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipRegUserTable_gh)
			sipRegUserTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipRegUserTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipRegUserTable_gh) {
			LOG(L_ERR, "%s: Error registering table handler\n", func);
			return -1;
		}
		/* it's not necessary for the caller to create this, but we need
		 * it to pass info down to the handling function */
		if(!sipRegUserTable_gh->sip_obj) {
			sipRegUserTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipRegUserTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipRegUserTable_gh);
				sipRegUserTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipRegUserTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipRegUserTable->table, row) != 
				SNMPERR_SUCCESS) {
			LOG(L_ERR, "%s: Error adding new row to table\n",func); 
			return -1;
		}
...
static int sipRegUserTable_newRow(struct sip_snmp_handler *h)
{
	 ...
	 * end of the linked list */
	c = h;
	for(i=2; i<SIPREGUSERTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPREGUSERTABLE_COLUMNS) {
...
/* only accepts read-only data */
int sipRegUserTable_replaceRow(
		struct sip_snmp_obj *idx,
		struct sip_snmp_obj *data)
{
	netsnmp_table_row *row;
	...
	/* find row */
	row = netsnmp_table_data_get(sipRegUserTable->table, idxs);
	if(!row) {
		LOG(L_ERR, "%s: Couldn't find row to replace\n", func);
		snmp_free_var(idxs);
		return -1;
...
/** handles requests for the sipRegUserTable table.
 * For every request it checks the specified object to see if it has a
 * handler, and calls it */
static int sipRegUserTable_handler(
		netsnmp_mib_handler               *handler,
		netsnmp_handler_registration      *reginfo,
		netsnmp_agent_request_info        *reqinfo,
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		/* this is not an error, since table-walks work by trying to get
		 * things until we run off of it */
		if(table_info->colnum > SIPREGUSERTABLE_COLUMNS)
			goto next;

		/* Get the handler and its object */
		if(sipRegUserTable_gh) {
			h = sipRegUserTable_gh;
			/* sip_obj is valid since we create upon registration */
			h->sip_obj->opaque = (void*)sipRegUserTable_replaceRow;
		} else {
			h = sipRegUserTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
