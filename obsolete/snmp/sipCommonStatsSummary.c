/*
 * $Id: sipCommonStatsSummary.c,v 1.2 2002/09/19 12:23:55 jku Rel $
 *
 * SNMP Module
 *
...
/* Table sipSummaryStatsTable */
static netsnmp_table_data_set* sipSummaryStatsTable;

static int initialize_table_sipSummaryStatsTable();
static Netsnmp_Node_Handler sipSummaryStatsTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipSummaryStatsTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipSummaryStatsTable_gh;
/* The registration function and friends */
static int sipSummaryStatsTable_reg(struct sip_snmp_handler *h, int op);
static int sipSummaryStatsTable_addObj(struct sip_snmp_handler *h, int col);
static int sipSummaryStatsTable_newRow(struct sip_snmp_handler *h);

/** Initializes the sipCommonStatsSummary module */
int init_sipCommonStatsSummary()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipSummaryStatsTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipSummaryStatsTable\n", func);
		return -1;
	}
	return 0;
}

/** Initialize the sipSummaryStatsTable table by defining it's contents and 
 * how it's structured */
static int initialize_table_sipSummaryStatsTable()
{
	static oid sipSummaryStatsTable_oid[] = {1,3,6,1,2,1,9990,1,5,1};
	size_t sipSummaryStatsTable_oid_len = OID_LENGTH(sipSummaryStatsTable_oid);
	const char *func = "snmp_mod";

	/* create the table structure itself */
	sipSummaryStatsTable = netsnmp_create_table_data_set("sipSummaryStatsTable");
	if(!sipSummaryStatsTable) {
	 ...
	 * Adding indexes
	 */
	netsnmp_table_dataset_add_index(sipSummaryStatsTable, ASN_INTEGER);
	
	netsnmp_table_set_multi_add_default_row(
		sipSummaryStatsTable,
		COLUMN_SIPSUMMARYINREQUESTS, ASN_COUNTER, 0, NULL, 0,
		COLUMN_SIPSUMMARYOUTREQUESTS, ASN_COUNTER, 0, NULL, 0,
		COLUMN_SIPSUMMARYINRESPONSES, ASN_COUNTER, 0, NULL, 0,
		COLUMN_SIPSUMMARYOUTRESPONSES, ASN_COUNTER, 0, NULL, 0,
		COLUMN_SIPSUMMARYTOTALTRANSACTIONS, ASN_COUNTER, 0, NULL, 0,
		0);
	
	/* registering the table with the master agent */
	/* note: if you don't need a subhandler to deal with any aspects
	 * of the request, change sipSummaryStatsTable_handler to "NULL" */
	netsnmp_register_table_data_set(
			netsnmp_create_handler_registration(
				"sipSummaryStatsTable", 
				sipSummaryStatsTable_handler,
				sipSummaryStatsTable_oid,
				sipSummaryStatsTable_oid_len,
				HANDLER_CAN_RWRITE),
			sipSummaryStatsTable, NULL);
...
{
	const char *func = "snmp_mod";
	sipSummaryStatsTable_h = calloc(SIPSUMMARYSTATSTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipSummaryStatsTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipSummaryStatsTable_reg;
}
...
static int sipSummaryStatsTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPSUMMARYSTATSTABLE_COLUMNS) {
		...
		if(!(sipSummaryStatsTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipSummaryStatsTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipSummaryStatsTable_h[col]);
			sipSummaryStatsTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipSummaryStatsTable_gh && first) {
			...
			c = h;
			if(SIPSUMMARYSTATSTABLE_INDEXES > 1) {
				for(i=1; i<SIPSUMMARYSTATSTABLE_INDEXES; i++)
					c = c->next;
			...
			while(c) {
				...
				if(col > SIPSUMMARYSTATSTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipSummaryStatsTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipSummaryStatsTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipSummaryStatsTable_gh)
			sipSummaryStatsTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipSummaryStatsTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipSummaryStatsTable_gh) {
		...
		if(!sipSummaryStatsTable_gh->sip_obj) {
			sipSummaryStatsTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipSummaryStatsTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipSummaryStatsTable_gh);
				sipSummaryStatsTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipSummaryStatsTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipSummaryStatsTable->table, row) != 
				SNMPERR_SUCCESS) {
...
static int sipSummaryStatsTable_newRow(struct sip_snmp_handler *h)
{
	...
	c = h;
	for(i=2; i<SIPSUMMARYSTATSTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPSUMMARYSTATSTABLE_COLUMNS) {
...
int sipSummaryStatsTable_replaceRow(
		...
		struct sip_snmp_obj *data)
{
	...
	/* find row */
	row = netsnmp_table_data_get(sipSummaryStatsTable->table, idxs);
	if(!row) {
...
/** handles requests for the sipSummaryStatsTable table.
 * For every request it checks the specified object to see if it has a
 * handler, and calls it */
static int sipSummaryStatsTable_handler(
		...
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		if(table_info->colnum > SIPSUMMARYSTATSTABLE_COLUMNS)
			goto next;

		/* Get the handler and its object */
		if(sipSummaryStatsTable_gh) {
			h = sipSummaryStatsTable_gh;
			/* sip_obj is valid since we create upon registration */
			h->sip_obj->opaque = (void*)sipSummaryStatsTable_replaceRow;
		} else {
			h = sipSummaryStatsTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
