/*
 * $Id: sipProxyStats.c,v 1.2 2002/09/19 12:23:55 jku Rel $
 *
 * SNMP Module
 *
...
/* Table sipProxyStatsTable */
static netsnmp_table_data_set* sipProxyStatsTable;

static int initialize_table_sipProxyStatsTable();
static Netsnmp_Node_Handler sipProxyStatsTable_handler;

/* The dynamic handler table */
static struct sip_snmp_handler** sipProxyStatsTable_h;
/* The global handler (if owner decides to handle this table that way) */
static struct sip_snmp_handler* sipProxyStatsTable_gh;
/* The registration function and friends */
static int sipProxyStatsTable_reg(struct sip_snmp_handler *h, int op);
static int sipProxyStatsTable_addObj(struct sip_snmp_handler *h, int col);
static int sipProxyStatsTable_newRow(struct sip_snmp_handler *h);

/** Initializes the sipProxyStats module */
int init_sipProxyStats()
{
	const char *func = "snmp_mod";
	/* here we initialize all the tables we're planning on supporting */
	if(initialize_table_sipProxyStatsTable() == -1) {
		LOG(L_ERR, "%s: Failed creating table sipProxyStatsTable\n", func);
		return -1;
	}
	return 0;
}

/** Initialize the sipProxyStatsTable table by defining it's contents and 
 * how it's structured */
static int initialize_table_sipProxyStatsTable()
{
	static oid sipProxyStatsTable_oid[] = {1,3,6,1,2,1,9991,1,4,1};
	size_t sipProxyStatsTable_oid_len = OID_LENGTH(sipProxyStatsTable_oid);
	const char *func = "snmp_mod";

	/* create the table structure itself */
	sipProxyStatsTable = netsnmp_create_table_data_set("sipProxyStatsTable");
	if(!sipProxyStatsTable) {
	 ...
	 * Adding indexes
	 */
	netsnmp_table_dataset_add_index(sipProxyStatsTable, ASN_INTEGER);
	
	netsnmp_table_set_multi_add_default_row(
		sipProxyStatsTable,
		COLUMN_SIPNUMPROXYREQUIREFAILURES, ASN_COUNTER, 0, NULL, 0,
		0);
	
	/* registering the table with the master agent */
	/* note: if you don't need a subhandler to deal with any aspects
	 * of the request, change sipProxyStatsTable_handler to "NULL" */
	netsnmp_register_table_data_set(
			netsnmp_create_handler_registration(
				"sipProxyStatsTable", 
				sipProxyStatsTable_handler,
				sipProxyStatsTable_oid,
				sipProxyStatsTable_oid_len,
				HANDLER_CAN_RWRITE),
			sipProxyStatsTable, NULL);
...
{
	const char *func = "snmp_mod";
	sipProxyStatsTable_h = calloc(SIPPROXYSTATSTABLE_COLUMNS+1,
		sizeof(struct sip_snmp_handler*));
	if(!sipProxyStatsTable_h) {
		LOG(L_ERR, "%s: Error initializing handler table: %s\n", 
			func, strerror(errno));
		return NULL;
	}

	return sipProxyStatsTable_reg;
}
...
static int sipProxyStatsTable_reg(struct sip_snmp_handler *h, int op)
{
	...
	if(op == REG_OBJ) {
		...
		col = h->sip_obj->col; 
		if(col <1 || col > SIPPROXYSTATSTABLE_COLUMNS) {
		...
		if(!(sipProxyStatsTable_h[col] = snmp_clone_handler(h))) {
			LOG(L_ERR, "%s: Error registering handler: %s\n", func,
				strerror(errno));
			return -1;
		}

		if(sipProxyStatsTable_addObj(h, col) == -1) {
			LOG(L_ERR, "%s: Error adding new object to table\n", func);
			snmp_free_handler(sipProxyStatsTable_h[col]);
			sipProxyStatsTable_h[col] = NULL;
			return -1;
		}
	} else if(op == NEW_ROW) {
		if(!h->sip_obj || !h->sip_obj->value.voidp) {
		...
		if(!sipProxyStatsTable_gh && first) {
			...
			c = h;
			if(SIPPROXYSTATSTABLE_INDEXES > 1) {
				for(i=1; i<SIPPROXYSTATSTABLE_INDEXES; i++)
					c = c->next;
			...
			while(c) {
				...
				if(col > SIPPROXYSTATSTABLE_COLUMNS) {
					LOG(L_ERR, "%s: Attempt to add too many objects to "
						"table row\n", func);
					return -1;
				}
				c->sip_obj->col = col;
				if(!(sipProxyStatsTable_h[col] = snmp_clone_handler(c))) {
					LOG(L_ERR, "%s: Error registering handler: %s\n", func,
						strerror(errno));
					return -1;
				}
		...
		/* add the data */
		if(sipProxyStatsTable_newRow(h) == -1) {
			LOG(L_ERR, "%s: Error creating new table\n", func);
			return -1;
		}
	} else if(op == REG_TABLE) {
		if(!sipProxyStatsTable_gh)
			sipProxyStatsTable_gh = snmp_clone_handler(h);
		else
			memcpy(sipProxyStatsTable_gh, h, sizeof(struct sip_snmp_handler));
		if(!sipProxyStatsTable_gh) {
		...
		if(!sipProxyStatsTable_gh->sip_obj) {
			sipProxyStatsTable_gh->sip_obj = calloc(1, 
					sizeof(struct sip_snmp_obj));
			if(!sipProxyStatsTable_gh->sip_obj) {
				LOG(L_ERR, "%s: Error registering table handler\n", func);
				free(sipProxyStatsTable_gh);
				sipProxyStatsTable_gh = NULL;
				return -1;
			}
		}
	} else {
...
static int sipProxyStatsTable_addObj(struct sip_snmp_handler *h, int col)
{
	 ...
	 * replaced */
	if(first) {
		if(netsnmp_table_data_add_row(sipProxyStatsTable->table, row) != 
				SNMPERR_SUCCESS) {
...
static int sipProxyStatsTable_newRow(struct sip_snmp_handler *h)
{
	...
	c = h;
	for(i=2; i<SIPPROXYSTATSTABLE_INDEXES; i++) 
		c = c->next;
	col = 1;
	while(c) {
		if(col > SIPPROXYSTATSTABLE_COLUMNS) {
...
int sipProxyStatsTable_replaceRow(
		...
		struct sip_snmp_obj *data)
{
	...
	/* find row */
	row = netsnmp_table_data_get(sipProxyStatsTable->table, idxs);
	if(!row) {
		...
		netsnmp_request_info              *requests) 
{
	...
	while(requests) {
		...
		if(table_info->colnum > SIPPROXYSTATSTABLE_COLUMNS)
			goto next;

		/* Get the handler and its object */
		if(sipProxyStatsTable_gh) {
			h = sipProxyStatsTable_gh;
			/* sip_obj is valid since we create upon registration */
			h->sip_obj->opaque = (void*)sipProxyStatsTable_replaceRow;
		} else {
			h = sipProxyStatsTable_h[table_info->colnum];
			if(!h) 
				goto next;
		}
		o = h->sip_obj;
